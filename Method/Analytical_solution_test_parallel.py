...
def my_pois(rate_a, rate_b, upper_lim, population):
...
def my_direct_pois_pval(rates_a, rate_b, scores, upper_lim, population):
    rates_a, inver = np.unique(rates_a, return_inverse=True)
    lams = .5 * rates_a * rate_b * population  # supposed to be .5
    res = []
    for lam in tqdm.tqdm(lams, total=len(lams), desc='individual pvals pois'):
        pvals = np.array([sum((poisson(lam).pmf(k + i) * poisson(lam).pmf(i)) for i in range(upper_lim))
                          for k in range(upper_lim)])
        # this is for one sided, for two sided by symetry:
        pvals[1:] *= 2
...
def my_direct_ind_pval(rates_a, rate_b, scores, population, branch_prop=None):
    ...
    # p = np.vstack([.5 * p_curr, 1 - p_curr, .5 * p_curr]).T
...
def RandomAtributes(tree, population, n_genes, mut_func):
    ...
    root = next(order)
    root.dat = sp.csr_matrix(np.random.randint(2, size=n_genes))
    for node in tqdm.tqdm(order, total=population, desc='propageting events on tree'):
        ...
        node.dat = np.abs(node.up.dat - sp.csr_matrix(mut_here))
    return tree
...
def RandomAtributesExact(tree, n_genes, homoplasy=1):
    ...
    for node in tqdm.tqdm(order, total=population, desc='propageting events on tree'):
        if lims is None:
            ...
            mut_here[idx_mut] = 1
        node.dat = sp.csr_matrix(np.abs(node_to_arr(node.up.dat) - mut_here))
    return tree
...
def MarginalMaxLiklihood(X, Tree, tip_row_dict, do_tips=False, naming=False, m_rate=0.5):
    ...
    wrap = lambda x: sp_to_arr(X[tip_row_dict[x.name]]) if x.is_leaf() and not do_tips else sp_to_arr(x.dat)
    tree_len = 0
    ...
    for i, node in tqdm.tqdm(enumerate(Tree.traverse('postorder')), total=tree_len,
        ...
        if node.is_leaf():
            if not do_tips:
                node.dat = X[tip_row_dict[node.name]]
            node.zero_prob = sp.csr_matrix(node.dat == 0)
            node.one_prob = sp.csr_matrix(node.dat == 1)
            continue
        ...
        node.dat = (node.zero_prob < node.one_prob).astype(int)
...
def JointMaxLiklihood(X, Tree, tip_row_dict, do_tips=False, naming=False, m_rate=0.5):
    ...
    for node in tqdm.tqdm(Tree.traverse('postorder'), total=tree_len, desc='Ancestral Reconstruction: 1st pass'):
        if node.is_leaf():
            ...
            node.C_0 = node.dat
            node.C_1 = node.dat
            node.L_0 = np.ones((node.dat).shape[1]) * (1 - m_rate)
            node.L_0[node.dat.indices] = m_rate
            node.L_1 = np.ones((node.dat).shape[1]) * m_rate
            node.L_1[node.dat.indices] = (1 - m_rate)
    ...
    """
    root.dat = sp.csr_matrix(root.L_0 < root.L_1)
    ...
    for i, node in tqdm.tqdm(enumerate(post), total=tree_len, desc='Ancestral Reconstruction: 2nd pass'):
        ...
        if not node.is_leaf():
            ...
            node.dat = sp.csr_matrix(node.C_0)
            node.dat[0, node.up.dat.indices] = node.C_1[node.up.dat.indices]
            # np.where(node.up.dat, node.C_1, node.C_0)
...
def GetPhyloSebsequentScore(tree, phenotree, phen_ind, skip=0, with_rand=False, dist_only=False, dist=None):
    ...
    """
    population = (len(tree) * 2) - 1
    subscore = np.zeros(tree.dat.shape[1] - skip)
    ...
    for i, (cur_node, phen_node) in tqdm.tqdm(enumerate(zip(tree.traverse(), phenotree.traverse())),
        ...
        if not cur_node.is_root():
            ...
            phen_state = phen_node.dat[0, phen_ind]
            prev_phen_state = phen_node.up.dat[0, phen_ind]
...
def GetPhyloParallelScore(tree, phenotree, phen_ind, skip=0, with_rand=False):
    ...
    score = np.zeros(tree.dat.shape[1] - skip)
    node_to_arr = lambda n: np.array(n.dat.todense().astype(np.int))[0]
    contingency_sum = 100
    for i, (cur_node, phen_node) in tqdm.tqdm(enumerate(zip(tree.traverse(), phenotree.traverse())),
                                              total=population, desc='Iterating tree'):
...
def GetBayesScore(tree, subset=None):
    ...
    PrintDat(tree, dat_path, [0], append=True)
    subprocess.call(BayesPhylo_path + ' ' + dat_path, shell=True, stderr=stdnull, stdout=stdnull)
    ...
    def tree_to_constraints(tree, output_path, dependant=False, MCMC=False, itNum=None, tol=None, stepstop=None):
        ...
        with open(output_path, 'w') as constraints_file:
            ...
            if not dependant:
                constraints_file.write('Res alpha1 beta1\n')
                constraints_file.write('Res alpha2 beta2\n')
            else:
                constraints_file.write('Res q12 q21\n')
                constraints_file.write('Res q13 q31\n')
                constraints_file.write('Res q24 q42\n')
                constraints_file.write('Res q34 q43\n')
            if stepstop is not None: constraints_file.write('MLTries ' + str(stepstop) + '\n')
    ...
    # tree_to_constraints(tree, indep_path, dependant=False, MCMC=False, itNum=10000, tol=0.001, stepstop=20)
    tree_to_constraints(tree, indep_path, dependant=False, MCMC=False, itNum=None, tol=None, stepstop=None)
    dep_path = nex_path + '_dependant.const'
    # tree_to_constraints(tree, dep_path, dependant=True, MCMC=False, itNum=10000, tol=0.001, stepstop=20)
    tree_to_constraints(tree, dep_path, dependant=True, MCMC=False, itNum=None, tol=None, stepstop=None)
    ...
    def BayesTraits(tree, phen_ind, gene_ind):
        ...
        # run independant
        subprocess.call(' '.join([BayesTraits_path, nex_path, dat_path, '<', indep_path]), shell=True, stderr=stdnull, stdout=stdnull)
        lastLine = tl.tail(open(dat_path + '.Log.txt'), 1)
        H_0 = float(lastLine[0][:100].split('\t')[1])

        # run dependant
        subprocess.call(' '.join([BayesTraits_path, nex_path, dat_path, '<', dep_path]), shell=True, stdout=stdnull, stderr=stdnull)
        lastLine = tl.tail(open(dat_path + '.Log.txt'), 1)
    ...
    if subset is not None:
        for i in tqdm.tqdm(subset, total=len(subset)):
            score.append(BayesTraits(tree, phen_ind=0, gene_ind=i))
        score_vec = pd.Series(index=range(tree.dat.shape[1]))
        score_vec.iloc[subset] = score
        score = score_vec.values[1:]
    else:
        for i in tqdm.tqdm(range(tree.dat.shape[1]-1), total=tree.dat.shape[1]-1):
            score.append(BayesTraits(tree, phen_ind=0, gene_ind=i+1))
...
def SetHomoplasy(tree, skip=0):
    ...
    order = tree.traverse()
    root = next(order)
    homoplasy_hist = sp.csr_matrix(np.zeros(root.dat.shape[1] - skip))
    for cur_node in tqdm.tqdm(order, total=population + 1):
        homoplasy_hist += (cur_node.dat[0, skip:] != cur_node.up.dat[0, skip:])
...
2) simulate dats with a (fixed or from poiss dist) number of events by subsampling branches of tree
3) select a first dat as phenotype
...
def pipe(n_mutations, population, outpath, random_prop=False, model_prop=False):
    ...
    #if random_prop:
    #    tree_size = np.array([x.dist for x in GroundTruthTree.traverse()][1:]).sum()
    #    for n in GroundTruthTree.traverse(): n.dist = (n.dist / tree_size)

    tree_size = 0
    for i, n in enumerate(GroundTruthTree.traverse()):
        n.id = i
        tree_size += n.dist
    population = (i - 1) if not random_prop else (tree_size - GroundTruthTree.dist)  # root is irrelevant
    ...
    for og_node, recon_node in zip(GroundTruthTree.traverse(), ReconTree.traverse()):
        ReconHist += (og_node.dat != recon_node.dat)
    ...
    # upper_lim = max(max(np.abs(tables_recon_para_score)) + 1, 10)
    ...
    # expected_pois = my_pois(m_rate, phen_rate, upper_lim, population)
    # mean_pois = my_pois((ReconTree.homoplasy_hist.mean() / population), phen_rate, upper_lim, population)
    ...
    # tables_recon['exp_pois_pval'] = expected_pois(tables_recon['score'])
    # tables_recon['mean_pois_pval'] = mean_pois(tables_recon['score'])
    # tables_recon['direct_pois_pval'] = my_direct_pois_pval((ReconTree.homoplasy_hist[1:] / population), phen_rate, tables_recon['score'].values, upper_lim)
    ...
    # 8.5) calculate score of annoying bayestraits models, sampling inverse prop to the pvalue
    print("8.5) calculate score of annoying bayestraits models, sampling inverse prop to the pvalue")
    # subset sample inverse prop
    subset = GroundTruthTables['pval'].sample(100, weights=1 / GroundTruthTables['pval']).index.tolist()
    tables_recon['bayes_score'] = GetBayesScore(GroundTruthTree, subset=subset)
    tables_recon['bayes_pval'] = chi2(2).sf(tables_recon['bayes_score'])

    if model_prop:
        branch_prop = [x.dist for x in ReconTree.traverse()][1:]
    ...
    tot_tables = GroundTruthTables.join(tables_recon, rsuffix='_recon').join(tables_sim, rsuffix='_treewas')
    if save_table:
